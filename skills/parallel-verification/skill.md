...
description: >
  AlphaGo-style parallel hypothesis exploration and verification.
  Use when: decision has high stakes with 3+ valid approaches, correctness matters more
  than speed, user says /pv or "parallel verify", or problem requires multi-perspective
  reasoning (architecture decisions, complex debugging, risk assessment).
  Don't use when: there's an obvious correct answer, task is time-sensitive,
  user wants a quick opinion (just answer), or problem has only one viable approach.
